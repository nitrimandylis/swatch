...
import glanceCssTpl from "./templates/glance-user.css" with { type: "text" };
...
/** `R G B`, sRGB floats at six decimals — how NSGlobalDomain stores a colour. */
export function srgb(hex: string): string {
  const at = (i: number) => (parseInt(hex.slice(1 + i * 2, 3 + i * 2), 16) / 255).toFixed(6);
  return `${at(0)} ${at(1)} ${at(2)}`;
}

/** `R G B A` — what the macOS icon tint stores. */
export const tintColor = (hex: string) => `${srgb(hex)} 1.000000`;

/**
 * `R G B Name` — what the system highlight colour stores. The trailing word is
 * the label System Settings prints under the swatch, not a lookup key: any name
 * is accepted and the colour comes from the floats.
 */
export const highlightColor = (hex: string, name: string) => `${srgb(hex)} ${name}`;
...
/** `H S L` with no units and no commas — how glance takes a colour. */
export function hsl(hex: string): string {
  const [r, g, b] = [0, 1, 2].map((i) => parseInt(hex.slice(1 + i * 2, 3 + i * 2), 16) / 255);
  const max = Math.max(r, g, b), min = Math.min(r, g, b), d = max - min;
  const l = (max + min) / 2;
  // Grey has no hue and no saturation, and the formula divides by zero on it.
...
/**
 * glance's `theme:` block, pulled into glance.yml with `$include` so swatch owns
 * the whole file. glance derives every text colour from these four plus
 * `contrast-multiplier`, so the multiplier is part of the theme, not a taste knob.
 */
export function glanceTheme(t: Theme): string {
  return [
    `# ${t.meta.name} — generated by swatch, do not edit`,
    `background-color: ${hsl(t.roles.base)}`,
    `primary-color: ${hsl(t.roles.accent)}`,
    `positive-color: ${hsl(t.ansi.green[1])}`,
    `negative-color: ${hsl(t.ansi.red[1])}`,
...
/**
 * fzf keeps its colours in one environment variable, so the surface is a shell
 * file a rc sources rather than a config swatch edits. It appends to whatever
 * FZF_DEFAULT_OPTS already holds, so `--height` and friends set elsewhere
 * survive; a later `--color` simply wins over an earlier one.
 *
 * `bg` and `gutter` stay at -1, meaning "inherit the terminal". Painting the
 * base in would draw an opaque rectangle over ghostty's transparency.
 */
export function fzfColors(t: Theme): string {
  const r = t.roles, a = t.ansi;
  const pairs: [string, string][] = [
    ["fg", r.text], ["bg", "-1"], ["hl", r.accent],
    ["fg+", r.text], ["bg+", r.overlay], ["hl+", r.accent],
    ["info", r.muted], ["border", r.overlay], ["header", r.muted],
    ["prompt", r.accent], ["pointer", r.accent], ["spinner", r.accent],
    ["marker", a.green[1]], ["gutter", "-1"],
  ];
  const spec = pairs.map(([k, v]) => `${k}:${v}`).join(",");
  return [
    `# ${t.meta.name} — generated by swatch, do not edit`,
    'export FZF_DEFAULT_OPTS="${FZF_DEFAULT_OPTS:-} --color=' + spec + '"',
    "",
  ].join("\n");
}
...
/** The generated web stylesheet, `@import`ed from Zen's userContent.css. */
export const WEB_FRAGMENT = "swatch-web.css";

/**
 * A gitconfig fragment for the colour sections, pulled in by one `[include]`
 * line. An include rather than markers because a `[section]` header inside an
 * injected block would silently adopt every key that follows the end marker.
 *
 * Hex values are quoted because `#` opens a comment in git's config syntax; the
 * parser strips the quotes and hands git `#rrggbb`, which it has understood
 * since 1.9.
 */
export function gitColors(t: Theme): string {
  const r = t.roles, a = t.ansi;
  const section = (name: string, keys: [string, string][]) =>
    [`[color "${name}"]`, ...keys.map(([k, v]) => `\t${k} = "${v}"`)].join("\n");
  return [
    `# ${t.meta.name} — generated by swatch, do not edit`,
    section("diff", [
      ["meta", r.muted], ["frag", r.accent], ["func", a.blue[1]],
      ["old", a.red[0]], ["new", a.green[0]], ["commit", a.yellow[0]],
    ]),
    // localBranch and remoteBranch are the two names in `git status -sb`. They
    // are not covered by `branch`, and left out they stay at git's default
    // green and red — the one place the palette visibly does not reach.
    section("status", [
      ["header", r.muted], ["added", a.green[0]], ["changed", a.yellow[0]],
      ["untracked", r.muted], ["branch", r.accent],
      ["localBranch", r.accent], ["remoteBranch", a.cyan[0]],
    ]),
    section("branch", [
      ["current", r.accent], ["local", r.text], ["remote", a.cyan[0]],
    ]),
    section("decorate", [
      ["branch", r.accent], ["remoteBranch", a.cyan[0]],
      ["tag", a.yellow[0]], ["HEAD", a.magenta[0]],
    ]),
    "",
  ].join("\n");
}
...
export const SURFACES: Surface[] = [
  ...
  },
  {
    name: "fzf",
    /**
     * Opt-in by directory. fzf has no config file of its own, so there is
     * nothing on disk that means "installed and themed" — an existing
     * `~/.config/fzf` is the user saying a shell rc sources what swatch writes.
     * Without that, `Bun.which("fzf")` would be true on any machine with the
     * binary and swatch would report a surface nothing reads.
     */
    apply(t) {
      const dir = join(CONFIG, "fzf");
      if (!existsSync(dir)) return null;
      put(join(dir, "colors.sh"), fzfColors(t));
      return "fzf (new shells only)";
    },
  },
  {
    name: "btop",
  ...
  },
  {
    name: "git",
    /**
     * Opt-in by include line. git reads both `~/.gitconfig` and
     * `~/.config/git/config`, so whichever one carries the include is the file
     * that asked for this; with neither, the fragment would sit on disk unread.
     */
    apply(t) {
      const homes = [join(homedir(), ".gitconfig"), join(CONFIG, "git", "config")];
      const opted = homes.some(
        (f) => existsSync(f) && readFileSync(f, "utf8").includes(GIT_FRAGMENT),
      );
      if (!opted) return null;
      put(join(CONFIG, "git", GIT_FRAGMENT), gitColors(t));
      return "git";
    },
  },
  {
  ...
  },
  {
    name: "web",
    /**
     * Sites with no themeable config of their own, themed inside Zen through
     * the CSS variables they already build their UI from. Notion's Mac app is
     * Electron with both wrappers dead, and so is Notion Calendar's; the rest
     * have no desktop client at all. DuckDuckGo does have a native
     * custom-theme setting, but it stores its colours in cookies with nothing
     * readable back, so `status` could not see it — the Raycast rejection.
     *
     * Opt-in by `@import`, the same shape as git's include: a Zen profile
     * existing does not mean you want the web restyled, and the generated
     * file's name never changes so there is no pointer line to rewrite.
     */
    apply(t) {
      const dir = zenProfile();
      if (!dir) return null;
      ...
      if (!existsSync(entry) || !readFileSync(entry, "utf8").includes(WEB_FRAGMENT))
        return null;
      put(join(chrome, WEB_FRAGMENT), render(webCssTpl, t));
      return "web (reload any open tab)";
    },
  },
  {
    name: "cider",
  ...
  },
  {
    name: "glance",
    /**
     * The glance dashboard. Its `theme:` block is a whole file swatch owns,
     * reached from glance.yml with `$include: theme.yml`, plus the stylesheet
     * that colours the contribution heatmap.
     *
     * No restart: glance watches its config and every included file and reloads
     * in place ("Config file changed, reloading..."), and user.css is served as
     * an asset, so a tab refresh picks it up.
     */
    apply(t) {
      const dir = join(CONFIG, "glance");
      if (!existsSync(join(dir, "glance.yml"))) return null;
      put(join(dir, "theme.yml"), glanceTheme(t));
      // ponytail: assumes `assets-path` points at ./assets, which is what the
      // docs tell you to set. Read it out of glance.yml if that ever varies.
      put(join(dir, "assets", "user.css"), render(glanceCssTpl, t));
      return "glance (reload the tab for the heatmap)";
    },
  },
  {
    name: "icons",
  ...
  },
  {
    name: "highlight",
    /**
     * The system selection colour, behind selected text in every native app.
     * `AppleHighlightColor` takes an arbitrary sRGB triple, so the palette goes
     * in exactly — unlike `AppleAccentColor`, which is an eight-value enum with
     * no slot for a custom colour, and is therefore left alone.
     */
    apply(t) {
      putDefault("AppleHighlightColor", highlightColor(t.roles.accent, t.meta.name));
      return "highlight (apps read it at launch)";
    },
  },
];
